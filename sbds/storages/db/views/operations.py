...
ALL_OPERATIONS_SQL = '''
    ...
    SELECT
        ...
        accounts,
        raw
    FROM sbds_op_virtual_hardforks
    UNION ALL
    SELECT
        block_num,
        transaction_num,
...
def create_all_ops_view():
    from sbds.storages.db.tables import metadata
    return view('sbds_views_all_ops', metadata, ALL_OPERATIONS_SQL)


def create_real_ops_view():
    from sbds.storages.db.tables import metadata
    return view('sbds_views_real_ops', metadata, REAL_OPERATIONS_SQL)


def create_virtual_ops_view():
    from sbds.storages.db.tables import metadata
    return view('sbds_views_virtual_ops', metadata, VIRTUAL_OPERATIONS_SQL)


CreateAllOpsView = create_all_ops_view()
CreateRealOpsView = create_real_ops_view()
CreateVirtualOpsView = create_virtual_ops_view()
