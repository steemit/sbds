...
class ChallengeAuthorityOperation(Base):
    ...
    __table_args__ = (ForeignKeyConstraint(
        ['challenger'], ['sbds_meta_accounts.name'],
        deferrable=True,
        initially='DEFERRED',
        use_alter=True),
        ...
        use_alter=True),
        Index(
        'ix_sbds_sbds_op_challenge_authorities_unique',
        'block_num',
        'transaction_num',
        'operation_num',
        unique=True),
        Index(
        'ix_sbds_op_challenge_authorities_accounts',
        'accounts',
    ...
    challenger = Column(Text, nullable=True)  # steem_type:account_name_type
    ...
    _fields = dict(
        accounts=lambda x: accounts_field(x, 'challenge_authority'), )
