...
class ProveAuthorityOperation(Base):
    ...
    __table_args__ = (
        ...
        Index('ix_sbds_op_prove_authorities_accounts', 'accounts', postgresql_using='gin')
    ...
    operation_type = Column(operation_types_enum, nullable=False, default='prove_authority')

    _fields = dict(
        accounts=lambda x: sbds.sbds_json.dumps(
            [acct for acct in set(flatten((x.get('challenged'),))) if acct])
    )
