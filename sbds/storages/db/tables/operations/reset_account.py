...
class ResetAccountOperation(Base):
    ...
    __table_args__ = (ForeignKeyConstraint(
        ['reset_account'], ['sbds_meta_accounts.name'],
        deferrable=True,
        initially='DEFERRED',
        use_alter=True),
        ForeignKeyConstraint(
        ['account_to_reset'], ['sbds_meta_accounts.name'],
        deferrable=True,
        initially='DEFERRED',
        use_alter=True),
        Index(
        'ix_sbds_sbds_op_reset_accounts_unique',
        'block_num',
        'transaction_num',
        'operation_num',
        unique=True),
        Index(
        'ix_sbds_op_reset_accounts_accounts',
        'accounts',
    ...
    reset_account = Column(Text, nullable=True)  # steem_type:account_name_type
    account_to_reset = Column(
    ...
    _fields = dict(
        ...
        accounts=lambda x: accounts_field(x, 'reset_account'),
    )
