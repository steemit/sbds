...
class CurationRewardVirtualOperation(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ['curator'], ['sbds_meta_accounts.name'],
            deferrable=True,
            initially='DEFERRED',
            use_alter=True),
        ForeignKeyConstraint(
            ['comment_author'], ['sbds_meta_accounts.name'],
            deferrable=True,
            initially='DEFERRED',
            use_alter=True),
        ...
        Index(
            'ix_sbds_sbds_op_virtual_curation_rewards_unique',
            'block_num',
            'transaction_num',
            'operation_num',
            text("MD5('raw')"),
            unique=True),
        Index(
            'ix_sbds_op_virtual_curation_rewards_accounts',
            'accounts',
    ...
    curator = Column(Text, nullable=True)  # steem_type:account_name_type
    ...
    _fields = dict(
        ...
        accounts=lambda x: accounts_field(x, 'curation_reward'),
    )
