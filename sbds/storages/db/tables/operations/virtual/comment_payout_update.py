...
class CommentPayoutUpdateVirtualOperation(Base):
    ...
    __table_args__ = (
        ...
        Index(
            'ix_sbds_sbds_op_virtual_comment_payout_updates_unique',
            'block_num',
            'transaction_num',
            'operation_num',
            text("MD5('raw')"),
            unique=True),
        Index(
            'ix_sbds_op_virtual_comment_payout_updates_accounts',
            'accounts',
    ...
    _fields = dict(
        accounts=lambda x: accounts_field(x, 'comment_payout_update'), )
