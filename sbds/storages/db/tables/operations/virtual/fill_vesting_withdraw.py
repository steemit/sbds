...
class FillVestingWithdrawVirtualOperation(Base):
    ...
    __table_args__ = (
        ...
        Index(
            'ix_sbds_sbds_op_virtual_fill_vesting_withdraws_unique',
            'block_num',
            'transaction_num',
            'operation_num',
            text("MD5('raw')"),
            unique=True),
        Index(
            'ix_sbds_op_virtual_fill_vesting_withdraws_accounts',
            'accounts',
    ...
    _fields = dict(
        ...
        accounts=lambda x: accounts_field(x, 'fill_vesting_withdraw'),
    )
