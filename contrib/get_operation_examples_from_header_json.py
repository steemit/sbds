...
@click.option('--outdir', type=click.STRING, default='')
def cli(infile, database_url, outdir):
    header = infile
    for operation_name, cls in header['classes'].items():
        op_example = get_op_example(operation_name, database_url)
        if op_example:
            filename = os.path.join(outdir,f'{operation_name}.json' )
